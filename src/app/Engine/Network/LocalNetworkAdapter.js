...
class LocalNetworkAdapter extends _NetworkAdapter {
      ...
      if (data.name == "OffsetGoldStash") {
        if (this.goldstash) {
          ...
          if (actualDx !== 0 || actualDy !== 0) {
            ...
            const isValid =
              this.fixOccurredBuildingsByType(newX, newY, "GoldStash") &&
            ...
            for (let i = 0; i < buildingUids.length; i++) {
              ...
              if (distX >= 865 || distY >= 865) {
                ...
                delete this.activeBuildingsByPos[
      ...
      if (data.name == "CastSpell") {
        if (data.type == "Tree" || data.type == "Stone") {
          if (
            ...
            data.y >= 0 &&
            data.y <= 24000 &&
            !this.newEntitiesByPos[data.x + ", " + data.y + ", " + data.type]
          ) {
            let _uid = ++this.uid;
        ...
        if (
          ...
          data.y >= 192 &&
          data.y <= 23808 &&
          this.fixOccurredBuildingsForRssByType(data.x, data.y, data.type)
        ) {
          let _uid = ++this.uid;
          let obj = {
            x: data.x,
        ...
        if (
          this.goldstash &&
          data.type !== "GoldStash" &&
          !this.activeBuildingsByPos[data.x + ", " + data.y] &&
          this.towersLength[data.type] < this.towerLimits[data.type] &&
          ...
          data.y >= 192 &&
          data.y <= 23808 &&
          this.fixOccurredBuildingsByType(data.x, data.y, data.type) &&
          this.fixOccurredBuildingsForRssByType(data.x, data.y, data.type)
        ) {
          let _uid = ++this.uid;
          let obj = {
  ...
  startMovementInterval() {
    ...
    this.moveInterval = setInterval(() => {
      ...
      this.playerdata.position.x = Math.max(
        0,
        Math.min(24000, this.playerdata.position.x),
      );
      this.playerdata.position.y = Math.max(
        0,
        Math.min(24000, this.playerdata.position.y),
      );
      if (
        this.playerdata.position.x !== oldX ||
        this.playerdata.position.y !== oldY
      ) {
        this.markEntityDirty(this.playerUid);
      }
    }, 16);
