...
class NetworkEntity extends _Entity {
    ...
    tick(msInThisTick, msPerTick) {
        if (this.fromTick) {
            ...
            this.setPositionX(_Util.lerp(this.fromTick.position.x, this.targetTick.position.x, tickPercent));
            this.setPositionY(_Util.lerp(this.fromTick.position.y, this.targetTick.position.y, tickPercent));
            this.setRotation(_Util.interpolateYaw(this.targetTick.yaw, this.fromTick.yaw));
        }
    }
    update(dt, user) {
        if (this.fromTick) {
    ...
    refreshModel(networkModelName) {
        ...
        if (!this.currentModel) {
            ...
            this.currentModel = _Game.currentGame.assetManager.loadModel(modelName, args);
