...
class UiIntro extends _UiComponent {
  constructor(ui) {
    ...
    this.statsElem = this.componentElem.querySelector(
      ".hud-intro-preview-stats",
    );
    this.coordsElem = this.componentElem.querySelector(
      ".hud-intro-preview-coords",
    );
    ...
    if (this.canvas) {
      ...
      this.canvas.addEventListener(
        "mouseleave",
        this.onCanvasMouseLeave.bind(this),
      );
  ...
  drawSpots(serverId, spots) {
    ...
    if (
      ...
      for (let id in spots) {
        ...
        if (
          Math.abs(spot.position.x - gameX) > margin ||
          Math.abs(spot.position.y - gameY) > margin
        ) {
          continue;
        }
  ...
  onCanvasMouseMove(event) {
    ...
    if (this.magnifierPopup) {
      const parentRect =
        this.canvas.parentNode.parentNode.getBoundingClientRect(); // .hud-intro-preview
  ...
  onSubmitClick(event) {
    ...
    if (
      ...
      this.errorElem.innerText =
        "This server is currently locked/unconfigured.";
      return;
    }
