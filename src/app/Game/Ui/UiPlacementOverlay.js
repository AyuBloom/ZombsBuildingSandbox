    ...
    if (this.buildingId) {
      ...
      if (!this.rangeIndicator) {
        if (this.buildingId === "GoldStash") {
          ...
          var range =
            (schemaData.rangeTiers && schemaData.rangeTiers[0]) ||
            (this.buildingId === "Harvester" ? 300 : 110);
          var maxYawDeviation =
            (schemaData.maxYawDeviationTiers &&
              schemaData.maxYawDeviationTiers[0]) ||
  ...
  placeBuilding() {
    if (this.buildingId) {
      ...
      for (var i in cellIndexes) {
        ...
        if (
          this.isOffsetting &&
          this.checkIsOccupied(cellIndexes[i], cellPos)
        ) {
          this.ui.components.PopupOverlay.showHint(
            "Cannot offset Gold Stash: target cells are occupied.",
            4000,
          );
          return false;
        }
      ...
      if (this.isOffsetting) {
        ...
        for (var uid in buildings) {
          ...
          if (building.uid === stash.uid || building.type === "Harvester")
            continue;
        ...
        var warningMsg =
          "Are you sure you want to offset the GoldStash by " +
          dx +
          ", " +
          dy +
          "?";
        if (outOfRangeCount > 0) {
          warningMsg =
            ...
            " building" +
            (outOfRangeCount > 1 ? "s" : "") +
            " that fall outside the new stash range. Proceed?";
        }
    ...
    for (var uid in entities) {
      ...
      if (networkEntity) {
        ...
        if (entityTick) {
          ...
          if (
            entityTick.model === "Tree" ||
            entityTick.model === "Stone" ||
            entityTick.model === "NeutralCamp"
          ) {
            var radius = 60;
