...
class HealTowersSpellModel extends _ModelEntity {
    constructor() {
        ...
        this.circle.drawCircle(0, 0, this.currentRadius, {
            r: 216,
            g: 0,
            b: 39
        }, {
    ...
    updatePulse() {
        ...
        this.circle.setAlpha(0.1 + Math.sin(this.currentPulse * 2 * Math.PI) * 0.05);
    }
    updateHearts() {
        for (var i = 0; i < this.heartTotal; i++) {
            if (this.hearts[i]) {
                ...
                this.hearts[i].setAlpha(0.5 - Math.min(1, this.heartOffsets[i] / this.heartMaxOffset) * 0.5);
                ...
                this.hearts[i] = new _SpriteEntity("/asset/image/entity/heal-towers-spell/heal-towers-spell-particle.svg");
