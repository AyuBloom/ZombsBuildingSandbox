...
class CharacterModel extends _ModelEntity {
    ...
    updateDamageTint(tick) {
        if (tick.lastDamagedTick && (tick.lastDamagedTick !== this.lastDamagedTick || !this.lastDamagedAnimationDone)) {
            ...
            var msSinceFiring = _Game.currentGame.world.getReplicator().getMsSinceTick(tick.lastDamagedTick);
            ...
            var tint = 255 - flashMultiplier * 255 / 4 << 8 | 16711680 | 255 - flashMultiplier * 255 / 4 << 0;
            if (flashPercent === 1) {
                tint = 16777215;
                this.lastDamagedAnimationDone = true;
            }
    ...
    updateBowWeapon(pullLength = 300) {
        return (tick, networkEntity) => {
            if (tick.firingTick && (tick.firingTick !== this.lastFiringTick || !this.lastFiringAnimationDone)) {
                ...
                var offsetPositionY = pullPercent < 0.75 ? 0.75 / pullPercent * 10 : 10 - 0.25 / (pullPercent - 0.75) * 10;
                if (pullPercent === 1) {
                    this.lastFiringAnimationDone = true;
                }
