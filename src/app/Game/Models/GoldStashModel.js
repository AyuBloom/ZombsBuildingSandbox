...
class GoldStashModel extends _ModelEntity {
  constructor() {
    ...
    this.base = new _SpriteEntity(
      "/asset/image/entity/gold-stash/gold-stash-t1-base.svg",
    );
  ...
  updateModel(tick, networkEntity) {
    if (tick.tier != this.currentTier) {
      ...
      switch (this.currentTier) {
        ...
        case 8:
          this.base = new _SpriteEntity(
            "/asset/image/entity/gold-stash/gold-stash-t" +
              this.currentTier +
              "-base.svg",
          );
          break;
        default:
          throw new Error(
            "Unknown tier encountered for gold stash: " + this.currentTier,
          );
      }
      this.addAttachment(this.base, 2);
    }
